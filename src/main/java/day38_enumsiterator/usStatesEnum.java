...
public enum usStatesEnum {
    ...
    // Enum'lar değişme ihtimali olmayan dataları depolamak için kullanılır. mesela gün isimleri, ay isimleri vs...
    //parantezin içine kaç tane değer oyulursa o kadar varieable oluşturması gereklidir.
    // Enumlar data type olarak kullanılır
    ...
    //State kısaltmasını verince state ismi veren kod
    public static String getStateNameByUsingAbbeviation (String abbreviation){
        return Arrays.stream(values())
                .filter(usState -> usState.getAbbreviation().equals(abbreviation))
                .findFirst().get().getStateName();
    }

    //State ismini verince state kısaltmasını veren kod
